...
// ═══════════════════════════════════════════════════════════════
// ATLAS-3: 434종 과실비율 분류 체계 (별첨 PDF 기반)
...
| 13 | 정체차로진로변경 | 진로변경(측면) | 직진(측면) | 100 | 0 |
| 14 | 안전지대(전) | 후행직진(미벗어남) | 선행진로변경 | 100 | 0 |
| 15 | 안전지대(후) | 후행직진(벗어남) | 선행진로변경 | 70 | 30 |
| 16 | 정차후출발 | 정차후출발 | 추월 | 80 | 20 |
| 17 | 긴급차(중앙선) | 직진 | 긴급차중앙선통행 | 60 | 40 |
| 18 | 긴급차(진로변경) | 후행직진 | 긴급차진로변경 | 90 | 10 |
| 19 | 긴급차(추월) | 선행직진 | 긴급차추월 | 60 | 40 |
| 20 | 긴급차(진로변경) | 선행진로변경 | 긴급차후행직진 | 100 | 0 |

### 사거리교차로 무신호 (유형 21~59)
| 유형 | 장소특징 | A | B | A과실 | B과실 |
|------|---------|---|---|-------|-------|
| 21 | 동일폭 | 우측직진 | 좌측직진 | 40 | 60 |
| 22 | 동일폭(후진입) | 우측직진(후) | 좌측직진(선) | 70 | 30 |
| 23 | 동일폭(선진입) | 우측직진(선) | 좌측직진(후) | 30 | 70 |
| 24 | 동일폭(대향좌회전) | 마주보며직진 | 마주보며좌회전 | 30 | 70 |
| 27 | 동일폭(우회전vs직진) | 우회전 | 직진 | 60 | 40 |
| 31 | 대로소로 | 대로직진 | 소로직진 | 30 | 70 |
| 32 | 대로소로 | 대로직진 | 소로좌회전 | 20 | 80 |
| 37 | 대로소로 | 소로우회전 | 대로직진 | 70 | 30 |
| 44 | 일시정지 | 표지없는도로직진 | 일시정지위반직진 | 20 | 80 |
| 51 | 일방통행 | 직진 | 일방통행위반 | 20 | 80 |
...
| 71 | 대향(적vs녹좌) | 적색직진 | 녹색좌회전 | 100 | 0 |
| 73 | 대향(녹vs녹좌) | 녹색직진 | 녹색좌회전(비보호X) | 0 | 100 |
| 79 | 비보호좌회전 | 녹색좌회전(비보호) | 녹색직진 | 80 | 20 |
| 91 | 유턴(상시) | 마주보며직진 | 상시유턴 | 20 | 80 |
| 92 | 유턴(신호) | 마주보며직진 | 신호유턴 | 100 | 0 |
...
/**
 * ATLAS-3: 434종 분류 체계 통합 Claude API
 * 사고 텍스트 → 도표 매칭 + 과실비율 분석
 */
...
위 434종 분류 체계를 참고하여, 해당 사고가 어떤 유형에 해당하는지 명확히 기술하세요.
